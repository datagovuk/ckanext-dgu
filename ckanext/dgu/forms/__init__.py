...
class FormAPI(SingletonPlugin):
    ...
    def after_map(self, map):
        map.connect('/api/form/package/create',           controller='ckanext.dgu.forms:FormController', action='package_create')
        map.connect('/api/form/package/edit/:id',         controller='ckanext.dgu.forms:FormController', action='package_edit')
        map.connect('/api/form/harvestsource/create',     controller='ckanext.dgu.forms:FormController', action='harvest_source_create')
        map.connect('/api/form/harvestsource/edit/:id',   controller='ckanext.dgu.forms:FormController', action='harvest_source_edit')
        map.connect('/api/1/form/package/create',         controller='ckanext.dgu.forms:FormController', action='package_create')
        map.connect('/api/1/form/package/edit/:id',       controller='ckanext.dgu.forms:FormController', action='package_edit')
        map.connect('/api/1/form/harvestsource/create',   controller='ckanext.dgu.forms:FormController', action='harvest_source_create')
        map.connect('/api/1/form/harvestsource/edit/:id', controller='ckanext.dgu.forms:FormController', action='harvest_source_edit')
        map.connect('/api/2/form/harvestsource/create',   controller='ckanext.dgu.forms:FormController', action='harvest_source_create')
        map.connect('/api/2/form/harvestsource/edit/:id', controller='ckanext.dgu.forms:FormController', action='harvest_source_edit')
        map.connect('/api/2/form/package/create',         controller='ckanext.dgu.forms:Form2Controller', action='package_create')
        map.connect('/api/2/form/package/edit/:id',       controller='ckanext.dgu.forms:Form2Controller', action='package_edit')
        return map
...
class BaseFormController(BaseApiController):
    ...
    def package_create(self):
        ...
        url = "http://%s:%s@%s/services/xmlrpc"%(
            config['dgu.xmlrpc_username'],
            config['dgu.xmlrpc_password'],
            config['dgu.xmlrpc_domain'],
        )
        log.info('Setting up XMLRPC proxy to %s', url)
        drupal = ServerProxy(url)
        res = drupal.user.get(user_id)
        log.info('Obtained Drupal name %r, publishers %r', res['name'], res['publishers'])
        try:
            ...
            # Get the fieldset.
            fieldset = self._get_package_fieldset(publishers=res['publishers'])
