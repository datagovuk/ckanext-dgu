...
class FeedbackController(BaseController):
    ...
    def report(self):
        ...
        items = model.Session.query(Feedback).\
            filter(Feedback.active==True)

        keys = ['total comments', 'total feedback', 'total users',
                'total private individuals', 'total organisations',
                'economic', 'social',
                'effective', 'other', 'linked']
        c.stats = {'now': datetime.now()}
        for k in keys:
            c.stats[k] = 0

        for item in items.all():
            c.stats['total feedback'] += 1
            if item.economic:
                c.stats['economic'] += 1
                c.stats['total comments'] += 1
            if item.social:
                c.stats['social'] += 1
                c.stats['total comments'] += 1
            if item.other:
        ...
        useritems = model.Session.query(Feedback).filter(Feedback.active==True).\
            distinct(Feedback.user_id)
        c.stats['total users'] = useritems.count()
        for u in useritems:
            if u.responding_as == 'organisation':
                c.stats['total organisations'] += 1
            else:
                c.stats['total private individuals'] += 1
        return render('feedback/report.html')
