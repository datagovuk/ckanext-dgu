...
class DguDateConvertError(Exception):
    pass


class DguDateType(object):
    '''Utils for handling dates in forms.
    * User inputs in form DD/MM/YYYY and it is stored in db as YYYY-MM-DD.

    LEGACY - ripe for using modern calls. However it does deal with partial
    dates, storing them in the db as partial dates (e.g. 2014-05 rather than
    2014-05-01 or something).
    ...
    def parse_timedate(cls, timedate_str, format_type):
        ...
        * If it can\'t understand the layout it raises DguDateConvertError
        '''
        ...
        for index, matcher in enumerate(cls.matchers[format_type]):
            ...
            raise DguDateConvertError("Cannot parse %s date '%s'. Acceptable formats: %s" % (format_type, timedate_str, acceptable_formats))
                ...
                if field == 'year':
                    if len(val) == 2:
                        ...
                        try:
                            ...
                            raise DguDateConvertError('Expecting integer for %s value: %s' % (field, val))
                        val = cls.add_centurys_to_two_digit_year(int_val)
                    elif len(val) == 3:
                        raise DguDateConvertError('Expecting 2 or 4 digit year: "%s"' % (val))
                if field == 'month':
    ...
    def iso_to_db(cls, iso_date, format):
        ...
        try:
            ...
            raise DguDateConvertError('Could not read date as ISO format "%s". Date provided: "%s"' % (format, iso_date))
    ...
    def format(cls, datetime_dict, format_type):
        ...
        for field in cls.datetime_fields:
            ...
            if val < min_ or val > max_:
                raise DguDateConvertError('%s value of "%s" is out of range.' % (field.capitalize(), val))
            if format_type == 'form':
                int_format_string = '%d'
            elif format_type == 'db':
